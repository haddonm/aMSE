...
3. How will the biological properties of each population be allocated? Will they be characterized as if they are linearly arranged along the simulated coast, or will their properties be allocated randomly? Without detailed spatial information about productivity and yield such allocation when mimicking a real fishery can take a lot of trial and error.
...
The abalone MSE package (__aMSE__) is designed to permit three different approaches to simulation testing. Each of these would requires a somewhat different initiation. The value of having these alternatives is that the amount of time taken to condition the model differs considerably between them.

### Unfished equilibrium - Purely hypothetical testing. 

This could be used when testing the sensitivity to variation or different conditions of a given harvest strategy 

* begin from an unfished equilibrium
* apply a given time-series of catches or harvest rates using a given harvest strategy throughout

### An Initial Hypothetical Depletion Level

Similar to version 1, but would include the sensitivity to initial conditions in any testing. It could answer the question of how well a harvest strategy operates at different depletion levels.

* begin from an unfished equilibrium
* apply a constant series of catches or a constant harvest rate to attain a given depletion level before applying the chosen harvest strategy.

### Condition the Operating Model on a Real Fishery

* begin from an unfished equilibrium
* apply an historically accurate level of fishing at the SAU scale, at least, and ideally at the population level (would require identifying populations using the GPS data logger data and using those area identifications to sub-divide the historical catches appropriately).
* first apply with respect to size, any known biological characteristics of weight, growth, maturity.and emergence to each population within each SAU.
* select plausible values for those constants, such as natural mortality, unfished recruitment, and unfished mature biomass
* then adjust productivity terms R0, B0, and others if required, so that the predicted dynamics of the individual SAU, under the historical catches reflect the observed dynamics. 
* if there is any knowledge of recruitment variation then include that as well.
* Use surplus production models or even size-structured models (if size-composition data is available) to gain insight into the relative productivity of each SAU.

It should be clear that option three should be most valuable to a particular fishery but also the hardest option to condition the operating model in an appropriate manner. 
...
The productivity characteristics can be illustrated by a simple plot (__`r fig_nums("f2",display="cite")`__). It is important to note that the productivity of each population differs as does the harvest rate that would produce the optimum yield from each population. In this case the depletion level that gives rise to the maximum production appears to be approximately the same, but this is not always the case. The implication of this is that attempting to manage the stock at the zone level would not permit the identification of a single optimum harvest rate for all blocks. The _MSY_ in each case is the peak of each curve in the plots with yield on the y-axis.

```{r fig.width=6, fig.height=5, echo=FALSE}
parset(plots=c(2,2))
maxy <- getmax(product[,"MatB",])
plot(product[,"AnnH",1],product[,"MatB",1],type="l",lwd=1,col=1,ylim=c(0,maxy),
     ylab="Mature Biomass",panel.first=grid(),xlab="Harvest Rate")
for (i in 2:numpop)lines(product[,"AnnH",i],product[,"MatB",i],col=i)

maxy <- getmax(product[,"Catch",])
plot(product[,"MatB",1],product[,"Catch",1],type="l",lwd=1,col=1,ylim=c(0,maxy),
     ylab="Yield",panel.first=grid(),xlab="Mature Biomass",xlim=c(0,1750))
for (i in 2:numpop)lines(product[,"MatB",i],product[,"Catch",i],col=i)

maxy <- getmax(product[,"Catch",])
plot(product[,"AnnH",1],product[,"Catch",1],type="l",lwd=1,col=1,ylim=c(0,maxy),
     ylab="Yield",panel.first=grid(),xlab="Harvest Rate")
for (i in 2:numpop)lines(product[,"AnnH",i],product[,"Catch",i],col=i)

maxy <- getmax(product[,"Catch",])
plot(product[,"Deplet",1],product[,"Catch",1],type="l",lwd=1,col=1,ylim=c(0,maxy),
     ylab="Yield",panel.first=grid(),xlab="Depletion")
for (i in 2:numpop)lines(product[,"Deplet",i],product[,"Catch",i],col=i)

```


`r fig_nums("f2", caption=" Four plots illustrating different aspects of the production for each population.")`

<br>

The values for the $MSY$, the $B_{MSY}$, the $H_{MSY}$, and related values of depletion,  are searched for within _product_ and then included into each population list within the  _zoneC_ object, which is why generating that object takes two steps.


```{r }
 # the structure of only one population is shown; the black lines in Figure 2
