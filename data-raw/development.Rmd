...
* Rather than use one data-file and one ctrl file, perhaps break up the data required into a file for PDF definitions by population, a file for the variables common across the region, and a control file.

* There needs to be a simpler implementation of larval dispersal this time through. This will have implications for any initial equilibrium conditions. See section.

* In an attempt to speed the model runs it will be worthwhile exploring the advantages and disadvantages of splitting the region into two, the constant part, regionC, and the dynamic part, regionD. Is the constant part always separate from the global part?

* Need to develop a new twoblock.csv file containing definitions for 6 populations rather then generalize by block. This would imply that the variance for eahc population would be smaller than that used for each block. 
