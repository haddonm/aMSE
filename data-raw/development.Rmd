...
* The zone is simply to container for all the SAU (there may only be one). The notion is that the variation of population properties within an SAU can differ, but conditioning the populations is a whole subject for discussion in itself.

* Running the MSE once conditioned should be simple, the difficult part is conditioning the model to represent a particular zone, there is a need to include the option of having larval dispersal.

* Currently using a control.csv file to identify conditioning files, HCR files, and related. There is a zone file containnig the strucutre of the simulated zone, and a datafile containing the conditioning data. 

* When constructing a zone the constant parts are kept in zoneC and the dynamic parts in zoneD. Is the constant part always separate from the global part? Certainly not having to move such large objects around all the time should speed things along.
...
* Currently using a spatial hierarchy zone/SAU/population instead of zone/block/population, so this is now more general than only Tasmanian terminology. Stick to this hierarchy throughout the code. Consistency.
...
* Once a particular run is constructed, but before the run proceeds, generate a series of plots and tables to characterize the simulated zone.
...
1) Initiate simulated zone - use a ctrl file to define details of data-file, HCR function, and other initial conditions. May involve setting an initial depletion level.

2) conduct assessment of zone status at start of each year (option for partial year?)
...
2) A data-file containing details relating to the whole zone
...
* nSAU   - number of Spatial Management Units  eg  2
...
The unfished equilibrium is a classical starting point for any model, even if a decision is made to begin any simulations at some lower depletion level. We are using a one-year time-step but the dynamics occurs within each year. The dynamic part of the zone has the following structure:


zoneD - the dynamic part of the zonal structure

* SAU     : num [1:6] 1 1 2 2 2 2
...
This represents the dynamics before any simulations have occurred, the unfished equilibrium state. Similarly, the constant part of the zone, made up of a list for each population, has the following structure:
...
In the code (_setupzone_) the sequence is:

1. first generate zoneC from the constants and zone1. At that point each population structure contains everything except those things relating to productivity: B0, ExB0, MSY, MSYDepl and popq. makezoneC also updates glb by including the move matrix.
2. running the _makezone_ updates B0, ExB0, and popq in zoneC and generates the initial equilibrium for zoneD. As this is the unfished equilibrium the catch, harvestR, and catchN matrices are empty.
3. The final run of _moderegC_ internally runs _doproductivity_ which allows zoneC's MSY and MSYDepl entries to be updated for each population, as well as producing the product matrix.


In the zoneD matrices, the numbers-at-size are the numbers at the start of the year, similarly, the matureB is the mature biomass at the start of the year. The ExB0 is the exploitable biomass implied by the initial numbers-at-size, which means to calcualte it we need to apply half of natural mortality and growth before multiplyiung by selectivity and weight-at-length.
