...
wkdir <- paste0(ddir,"/AbMSE/AbMSE/data-raw/")
...
$$q_p=\exp \left[ \sum\limits_{t=1}^{n}{Ln\left( {{I}_{p,t}}/B_{p,t}^{E} \right)}/n \right]$$
...
Recruitment is modelled each year for each population using the Beverton and Holt stock recruitment relationship as re-parameterized by Francis (1990).
...
This requires, for each population, that $h$, $R_0$, $B_0$, and $\sigma_R$ be defined, and for each year that ${B_{p,t}^{S}}$ be estimated. Should deterministic recruitment be required (as when calculating the unfished, equilibrium zone structure), then set $\sigma_R$ to a very small number (when 1e-08 is squared this is the smallest number possible with 15 significant digits).
...
The assumption is made that the sum of the catches across the SAU equates to the TAC. If ever the TAC were not completely caught this assumption would obviously have to change. Each year the aspirational catches for each SAU will be determined by application of the harvest control rule for ewhatever harvest strategy is being used. In Tasmania, this is uses a multi-criterion decision analysis based around different aspects of cpue. In the operating model this is implemented initial without error. 

$$C_{u,t,hcr}={C_{u,t-1}} \times multhcr$$

where $C_{u,t-1}$ is the actual catch from SAU $u$ in year $t-1$, $C_{u,t,hcr}$ is the aspirational catch from SAU $u$ in year $t$, and $multhcr$ is the multiplier derived from whatever HCR is used.

The new $TAC_t$ will be:

$$TAC_t={\sum\limits_{u=1}^{nSAU}C_{u,t,hcr}}$$

The first step on the way to setting the potential actual SAU catch is to include Log-Normal variation to the SAU exploitable biomass on the assumption that the divers gain an appreciation of how much is present in each SAU but they make mistakes and so mis-allocate effort:
...
$\sigma_B$ is the standard deviation of the variation that occurs between the real exploitable biomass and the perceived distribution of exploitable biomass across SAU's, which also includes other sources of uncertainty.
...
where $C_{u}^*$ is the actual SAU catch scaled to the new TAC.
...
Haddon, M. (2021) _Using R for Modelling and Quantitative Methods in Fisheries_. The R Series. CRC/Chapman & Hall.  337p.
...
R Core Team (2020). _R: A language and environment for statistical computing._ R Foundation for Statistical Computing, Vienna, Austria. URL https://www.R-project.org/.
