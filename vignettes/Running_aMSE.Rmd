...
library(aMSE)  # you might check out the help pages for each function
 ...
 resdir <- outdir$resdir   # already exist
...
# estimates a revised MSY and related statistics.
ans <- findunfished(regionC,regionD,glb) # can take a while
regionC <- ans$regionC  # region constants
regionD <- ans$regionD  # region dynamics
product <- ans$product
...
```{r echo=FALSE}
suppressWarnings(kable(unfishprops,digits=c(4,4,4,4,4,4,4)))
