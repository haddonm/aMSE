...
Management strategy evaluation (MSE) entails the construction of a simulation framework that encompasses a simulation of the dynamics of the fishery being considered (the operating model), which must include a simulation of sampling from that fishery. Finally, it requires a simulation of the management or harvest strategy that is to be applied to the fishery, including what data is used, what assessment or performance measures are used, and what harvest control rule is applied (__`r fig_nums("f1",display="cite")`__). Both blacklip and greenlip abalone are species whose geographical distributions have high degrees of spatial complexity, meaning that both abalone biology (growth, maturity, productivity) and the fishery (catches, catch-rates, and resilience to fishing) varies greatly at relatively small geographical scales. Such spatial complexity is one reason why large scale stock assessment models have limited success with abalone species; evidenced by their failure to provide more successful management advice where they have been applied (NSW, Victoria, and New Zealand) than where management has been based upon more upon empirically based performance measures. 
...
`r fig_nums("f1", caption=" A representation of the main components of an MSE simulation framework as used with abalone (Haddon and Mundy, 2016).")`

<br>

## Two Potential Approaches

Being a simulation, running an MSE to explore options with in a particular fishery can take a number of different approaches. The first, and most obvious, is to attempt to simulate a real fishery as closely as possible and project its dynamics forward under the alternative harvest strategies that are to be compared. By conditioning the MSE to a particular fishery then presumably one would begin any projections from close to the actual current depletion state. Such an approach should provide the most convincing comparisons of the different harvest strategies (_HS_) for a particular fishery. This would enable workers to determine how successful each possible _HS_ was in the future management of that particular stock.

A second approach is more general because it is less focussed on a particular fishery. Instead, one attempts to simulate a generic fishery for species of a particular type, that have a type of life-history and a type of productivity seen across a range of fisheries. Thus, for example, instead of setting up the MSE to reflect the fishery for blacklip abalone on the west coast of Tasmania one might set it up to reflect a 'typical' blacklip abalone fishery. If it is possible to condition the simulation model to reflect such a generic fishery then one could artificially deplete the simulated stock to a given level. Thus, one might try to determine how well different _HS_ could recover a depleted stock, maintain a relatively productive stock, and manage a fish-down from an undepleted state to a more productive state (__`r fig_nums("f2",display="cite")`__).
...
`r fig_nums("f2", caption=" Two alternative approaches for initiating and using the MSE framework.")`

<br>

A common strategy used when applying MSE to a particular fishery is first to fit the operating model dynamics to historical data from the particular fishery as if it were a stock assessment model (refs). An advantage of this is that the operating model is put into the best available estimate of the current state of the fishery prior to applying alternative management approaches (harvest strategies). Any differences then found in the effectiveness of the alternative management approaches can then be attributed  with more confidence to the effects of that management rather than to the initial state of the operating model. Thus, the initial conditioning of the operating model is clearly important. Unfortunately, the biology and fishery for abalone is so spatially complex that any assessment based on a large geographical scale can only be an approximation at best. This means that the initial conditioning of the operating model in the abalone MSE remains a difficult problem. Before describing how this conditioning might proceed we will first examine the assumed spatial structure within the operating model.

## The Underlying Spatial Structure and Population Dynamics

The spatial complexity of abalone fisheries is the main reason that structurally simple stock assessment models fail to capture the dynamics of abalone fisheries. Hence, an explicit spatial structure is required for the operating model. Here, the upper geographical scale being used is that of a zone, which is the scale at which quota or TACs are set. In Tasmania this is equivalent to the quota zone. Within the single zone, there will be (can be) multiple spatial assessment / management units (SAUs) for which aspirational catches are set (which sum to the TAC; in Tasmania these are equivalent to the statistical blocks), and within each SAU there can be multiple populations (persistent areas of productivity) of abalone, each with their own set of biological properties, which reflect, to some extent, the average properties for the SAU.

Within each population the population dynamics are described using difference equations operating at an annual time-step with the biological processes of growth, mortality, and recruitment occurring in a modelled sequence. The dynamics in this size-based model are described by following the fate of the numbers-at-size. In each year the dynamics begin with the numbers-at-size at the start of each year. The first part of the annual process is that growth occurs, then half natural mortality is applied. At that point the exploitable biomass can be calculated from the numbers-at-size before fishing occurs (and it may well be greater than the spawning biomass, which is calculated at the start of the year before growth has occurred). The remaining dynamics involve the removal of the catch, the application of the last half of natural mortality and the addition of recruits, at that point larval dispersal occurs (if it is > 0.0), which produces the numbers-at-size at the start of the next year.

## Model Initiation

Before making any simulation runs it is first necessary to initiate the model/simulation. Here that initiation entails first reading in the data files describing the biological properties of the various populations making up the simulation and deriving the numbers-at-size and all the associated statistics and properties that enable the dynamics of the fishery to be described. The initial populations are established as unfished populations in an equilibrium derived from the individual recruitment levels of each population, their associated growth and maturity properties and the natural mortality rate. At the same time the productivity of each population and the total zone is characterized. Once this zone is produced it can then be conditioned ready for any projections to be made (__`r fig_nums("f2",display="cite")`__).


##One Possible Workflow
### The R Package Requirements

To implement a run using the __aMSE__ R package one currently needs to install  __aMSE__, __rutilsMH__, and __makehtml__, all of which can be downloaded or installed from the GitHub sites:  

https://github.com/haddonm/aMSE, https://github.com/haddonm/rutilsMH, and  https://github.com/haddonm/makehtml 
...
Apart from __rutilsMH__, the __aMSE__ package now uses the new package __makehtml__ that contains functions that allow for the rapid local website generation of results from a particular run. It is thus currently necessary to also install that package, it is not necessary to generate the internal website but __makehtml__ also provides the functionality to generate and store .csv files of tables and .png files of plots, all in one directory.

Apart from installing the R packages __aMSE__, __rutilsMH__, and __makehtml__, for each run of the MSE you will need to identify and name one or two directories somewhere on your computer, here we will refer to them as the _datdir_ and the _resdir_. The first will contain all required data (__datdir_), and the second will be ready to receive all the desired results (__resdir__); the same directory can serve as both. You will need such a _resdir_ for each MSE run that you wish to remain separate (for example, containing the outcomes from different harvest strategies, for later comparison). Each such result directory will be the focus for all the stored results from each particular run. The idea is that given the _resdir_ it will be possible to access all the plots, tables, and other results generated from the given run (indeed it should be possible to repeat the entire run if required). Later functions will be written that will take a set of such run directories and summarize and compare the results found within.

In the _datdir_ directory (which can be, and most often should be, the same as the _resdir_) there needs to be at least four .csv files:
...
Additional files may contain conditioning data, such as historical catches, cpue, and/or size-composition data. 

Functions are provided within __aMSE__ for writing out template versions of the first three files. These mimic the worked example we will use below and have been used to generate some of the built-in data-sets within __aMSE__, which are used to illustrate the operation of the various R functions that have been developed for conducting the simulations. In practice, one would run the _ctrlfiletemplate_, the _zonefiletemplate_, and the _datafiletemplate_  R functions to produce such files and then use them after editing the contents to suit your own fishery and particular run details (such editing of the biological and fishery properties would constitute the initial the conditioning of the operating model). 

You should not be surprised to find that just as there are _xxxxfiletemplate_ functions there are _readctrlfile_, _readzonefile_, and _readdatafile_ R functions (though eventually these may be hidden within more all encompassing functions). The intent (hope) is that most users should eventually not need to know such details.

The _resdir_ directory (referred to in the R code as _resdir_) is the repository for all the plots and tables that are produced by __aMSE__ for a particular run. When the directories for a particular run are setup and checked (using _setuphtml_), a _resultTable.csv_ file is also created. After that, if desired, each plot or table that are generated can be stored as a '.png' or '.csv' file in the _resdir_, and the filename of each result logged by adding it to the _resultTable.csv_ (the function _logfilename_ can be used to add the filename to the list). Actually, it is best to use the functions _addtable_ and _addplot_ to do the detailed work for you. At the same time one should add a category (productivity, catches, etc.) and caption (see the help page for _logfilename_, _addplot_, and _addtable_). At any stage in the run, it is then possible to call the function _make_html_ and this will produce a local web-page inside the _resdir_ and one can open it in a local browser (or it can open automatically). This provides for a quick way to visualize the results from a run and allow for the planning of later comparisons. The syntax for the _make_html_ function can be obtained easily using the function _endmakehtml_.

## Creating an Unfished Equilibrium Zone

There are a number of steps involved when initiating the __aMSE__ simulation framework: 

### Structural and Implementation Decisions

Initial decisions on the run directory, filenames for the zone, data, and HCR files, the spatial structure to be simulated, the zone wide properties (such as larval dispersal rate), the number of simulations to run, the number of years to project the simulation, and other aspects relating to the details of a particular files used in the run (see the format and details of the _ctrl_ and _zone_ objects and data files).

### Conditioning on Biological Characteristics

Biologically condition the operating model on a fishery using biological characteristics taken from the fishery, or, alternatively, use a hypothetical set of characters as a testbed (__`r fig_nums("f2",display="cite")`__).
...
Essentially it is a matrix of of _numpop_ matrices with the exploitable biomass, the mature biomass, the effective harvest rate, the catch or yield, the depletion level, and the relative cpue. Each row of values is obtained by running the dynamics with each constant harvest rate (listed in the rownames) until an equilibrium is reached. Thus, by searching for the maximum equilibrium or sustainable yield one can characterize what is required to obtain the $MSY$, the $B_{MSY}$, the $H_{MSY}$ and what level of stock depletion will be expected to provide the circumstances that can lead to the maximum sustainable yields.

These values are searched for and then included into each population list within the  _zoneC_ object, which is why generating that object takes two steps.
...
# addtable saves a copy pf unfishprops into resdir
```
