...
Apart from __rutilsMH__, the __aMSE__ package now uses the new package __makehtml__ that contains functions that allow for the rapid local website generation of results from a particular run. It is thus currently necessary to also install that package, though it is intended to make its use optional.

Apart from installing the R packages __aMSE__, __rutilsMH__, and __makehtml__, for each run of the MSE you will need to identify and name two directories somewhere on your computer, here we will refer to them as the _datdir_ and the _resdir_, which will contain all required data (__datdir_), and be ready to receive all the desired results (__resdir__); the same directory can serve as both. You will need such a _resdir_ for each separate MSE run that is made. Each such result directory will be the focus for all the stored results from each particular run. The idea is that given the _resdir_ it will be possible to access all the plots, tables, and other results generated from the given run (indeed it should be possible to repeat the entire run if required). Later functions will be written that will take a set of such run directories and summarize and compare the results found within.
