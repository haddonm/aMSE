...
readctrlfile <- function(rundir,infile="control.csv",verbose=TRUE) {
 ...
 #  batch <- getsingleNum("batch",indat)  # deprecated
   ...
   gauntlet <- getsingleNum("SLOTFISHERY",indat) # SLOTFISHERY--------
   if (is.null(gauntlet)) gauntlet <- 0
   sauLML <- getsingleNum("sauLML",indat)        # complements slotfishery
   ...
   if (projyrs > 0) {   # Projection Definitions -------------------------
     ...
     if ((!gauntlet) & (!sauLML)) {  # No slotfishery
       from <- grep("PROJLML",indat)
       for (i in 1:projyrs) {
         from <- from + 1
     ...
     if ((!gauntlet) & (sauLML)) {   # No slotfishery, SAU with different LML
       ...
       for (i in 1:projyrs) {
         from <- from + 1
         projLML[i,] <- c(getConst(indat[from],nb=nSAU,index=2), # puts maxsize
                          rep(maxsize,nSAU))  # for each sau and year
       }
     }
     if ((gauntlet) & (sauLML)) {  # A slotfishery and potentially different
       from <- grep("PROJLML",indat)   # LML for each SAU
       for (i in 1:projyrs) {
         from <- from + 1
...
#' @title updatedeltarec if recruitment timevarys extend across the varyrs
#'
#' @description updatedeltarec if a linear change in recruitment is modelled
...
#'
#' @param glb the globals object generated by readctrlfile and makezone
#' @param varyrs how many years at the end of conditioning on the fishery
...
#'
#' @returns a revised copy of glb the globals object
#' @export
#'
#' @examples
#' print("Wait on date sets")
updatedeltarec <- function(glb,varyrs) {
  deltarec <- glb$deltarec
  if (is.null(deltarec)) return(glb)
  n <- length(deltarec)
  finalprop <- deltarec[n]
  yrnum <- as.numeric(names(deltarec))
  recnum <- (yrnum[1] - varyrs):yrnum[n]
  nrec <- length(recnum)
  newdelta <- seq(1.0,finalprop,length=nrec)
  names(newdelta) <- recnum
