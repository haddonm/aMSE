...
#'
#' @description make_html writes a set of HTML files with tabbed navigation
...
#' @param resdir Directory where a particular run's resulst are all held.
#'     It must contain a sub-directory called plots where *.png files
#'     are saved
#' @param plotFileTable CSV file with info on PNG files. The resdir/plot
#'     directory will be sequentially searched for filenames included
#'     in this csv file, which includes a figure legend and a category=tab,
#'     which identifies in which tab the plot or table will be placed.
...
make_html <- function(replist=NULL,
                      ...
                      verbose=TRUE) {
# Clarify data
  plotdir <- filenametopath(resdir,"plots")
