...
#'
#' @return a list of the three PM scores, and the total scores, the TAC
#'     multipliers, and the Target cpue target and limit.
#' @export
#'
#' @examples
#' print("wait on example data being available")
calibrateMCDA <- function(histCE,saunames,hsargs,mult=1.1) {
  # histCE <- zone$zone1$condC$histCE; saunames=zone$zone1$SAUnames; mult=1.1; hsargs=HSargs
#  for (i in 1:length(hsargs)) assign(names(hsargs)[i],hsargs[[i]])
  ...
  for (sau in 1:nSAU) { # sau=1
    ...
    grad1val[,sau] <- tmp
    score1[,sau] <- getscore(grad1val[,sau],mult=mult)
    tmp2 <- getgrad4(histCE[pickce,sau],wid=hsargs$wid)
    nec2 <- length(tmp2)
    if (nec2 < yrce) tmp2 <- c(rep(NA,(yrce-nec2)),tmp2)
    grad4val[,sau] <- tmp2
    score4[,sau] <- getscore(grad4val[,sau],mult=mult)
    tmp3 <- targscore(histCE[pickce,sau],qnt=hsargs$targqnt,mult=mult)
  ...
  return(list(score1=score1,score4=score4,scoret=scoret,scoretot=scoretot,
              multTAC=multTAC,tdetails=tmp3$details))
...
#' score1 <- getscore(grad1)
#' cbind(blockE13$year[2:nyr],grad1,score1)
getscore <- function(pm,mult=1.1) {   # pm=scoret$ans[,"targval"]; mult=1.1
...
mcdahcr <- function(vectce,yr,hsargs) {
 ...
 # for (i in 1:length(hcrargs)) assign(names(hcrargs)[i],hcrargs[[i]])
  ...
  score1 <- getscore(grad1)
  scoret <- targscore(vectce,qnt=hsargs$targqnt)
  pmwts <- hsargs$pmwts
